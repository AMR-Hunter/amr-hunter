...
Intermediate tables are written to `reports/publication/`; figures are regenerated by the
`scripts/plot_*.py` scripts and are not committed to the repository.
...
| Figure 2A–E (CRyPTIC validation panels) | `scripts/plot_cryptic_validation_figures.py`, `scripts/make_fig2_combined.py` | regenerated locally |
...
| Internal cohort figures (five-category re-interpretation) | `scripts/plot_local_cohort_figure.py`, `scripts/plot_local_alluvial_figure.py`, `scripts/refresh_local_five_category.py` | regenerated locally |
